...
class Agent:
    ...
    def decide_random_move(self, world_size, current_x, current_y, step):
        
        self.visited.add((current_x, current_y))
        if (current_x, current_y) not in self.path_stack:
            self.path_stack.append((current_x, current_y))
    ...
    def decide_move(self, world_size, current_x, current_y, step):
        ...
        zero_prob_neighbors = [nbr for nbr, prob in probabilities.items() if prob == 0]
    
        if zero_prob_neighbors:
            safest_cell = zero_prob_neighbors[0]
            self.backtrack_steps = 0
            self.backtracked_states.clear()
        else:
            if not self.path_stack:
                fallback_neighbors = list(self.bayesian_network.get_valid_neighbors(current_x, current_y))
                safest_cell = (
                min(probabilities, key=probabilities.get)
                if probabilities else random.choice(fallback_neighbors)
                if fallback_neighbors else (current_x, current_y)
                )
            elif self.backtrack_steps < (world_size // 2) - 1:
                candidate = self.path_stack[-1]
                if candidate in self.backtracked_states:
                    fallback_neighbors = neighbors if neighbors else list(self.bayesian_network.get_valid_neighbors(current_x, current_y))
                    safest_cell = (
                    min(probabilities, key=probabilities.get)
                    if probabilities else random.choice(fallback_neighbors)
                    if fallback_neighbors else (current_x, current_y)
                    ...
                    self.path_stack.pop()  # Remove current cell from stack.
                    if self.path_stack:
                        safest_cell = self.path_stack[-1]
                        self.backtracked_states.add(safest_cell)
                    else:
                        fallback_neighbors = list(self.bayesian_network.get_valid_neighbors(current_x, current_y))
                        safest_cell = (
                         min(probabilities, key=probabilities.get)
...
if __name__ == "__main__":
    ...
    #which_move = int(input("Enter 1 for best move, or 2 for random move: "))
    ...
    while True:
        ...
        if 'Pit' in status:
            ...
            world.performance -= 1000 
            agent.evidence[f"Pit_{current_x}_{current_y}"] = 1
            break
        if 'Wumpus' in status:
